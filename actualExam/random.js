...
/* The general contract of next is that it returns an int value and if the argument bits is between 1 and 32 (inclusive), 
   ...
   The method next is implemented by class Random by atomically updating the seed. */
    ...
    // returns a copy of an array, "cut" like a deck of cards.
    // a random point is chosen, then the array is sliced at that point, and reassembled.
    
    this.cut = function (someArray) {
        if (!someArray.length)
            return null;
	var cutPoint = this.nextIntRange(someArray.length);
        return someArray.slice(cutPoint).concat(someArray.slice(0,cutPoint));
    }
    ...
    this.intBetween = function(firstVal,lastVal)  {
	var range = lastVal - firstVal + 1;
	return firstVal + this.nextIntRange(range);
    }
